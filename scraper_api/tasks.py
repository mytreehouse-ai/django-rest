...
def lamudi_multi_page_scraper_task():
    ...
    if not scrapy_jobs:
        ScrapyJobModel.objects.filter(single_page=False, is_multi_page_processed=True).update(
            is_multi_page_processed=False,
...
def create_slug():
    for city in CityModel.objects.filter(slug__isnull=True)[:50]:
        city.slug = slugify(f"{city.name}_{city.id}")
...
def reset_single_page_processing_status():
    ...
    # If there are no such Scrapy jobs, reset the single-page processing status for all jobs with HTML code
    if scrapy_job_count == 0:
        ScrapyJobModel.objects.filter().update(
            is_single_page_processed=False
        )
