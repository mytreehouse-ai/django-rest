...
class ReadFilteredCityAPIView(ListAPIView):
    """
    API view for reading filtered cities.

    This view allows for listing filtered cities with support for searching and ordering.
    ...
    queryset = DomainService.get_filtered_city()

    @swagger_auto_schema(
        operation_description="Retrieve a list of filtered cities. Supports searching by city name and ordering by id, created_at, and updated_at.",
        operation_id="list_filtered_cities",
        tags=["Domains"],
    )
    def get(self, request, *args, **kwargs):
        """
        Handles GET requests to retrieve a list of filtered cities.
